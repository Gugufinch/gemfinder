...
**Reading guide for implementers:**

- The full code for Chunks 1, 2, 3, 4.4, and 5.1 is inlined in this plan — copy directly into the files.
- Chunks 4.2 (scoring), 4.3 (enrichment), 5.2-5.4 (other routes), and 6 (UI) reference specific sections of the spec doc by line range. The spec contains complete code blocks for these. **The implementer (or executing subagent) must read the spec sections directly to obtain copy-pasteable code.**
- The spec doc is at: `docs/superpowers/specs/2026-05-12-hunter-v1-design.md`
- Don't try to re-derive these implementations from prose — read the spec's TypeScript blocks for exact field shapes, function signatures, and error handling patterns.
...
### Task 4.4: Orchestrator (the pipeline runner)

**Files:**
- Create: `lib/gemfinder/hunter/orchestrator.ts` + `.test.ts`

This is the highest-risk piece — full pipeline runner with concurrency, error isolation, and run-state transitions. Code inlined here (do NOT just reference the spec).

- [ ] **Step 1: Write failing test (smoke-level test)**
...
vi.mock('./musicbrainz', () => ({
  searchArtists: vi.fn(async () => ([
    { id: 'mbid-1', name: 'Test One', country: 'US', type: 'Person', tags: [{ name: 'indie pop', count: 5 }], relations: [], 'release-groups': [{ id: 'rg', title: 'X', 'first-release-date': '2023-01-01' }] },
  ])),
}));
vi.mock('./spotify', () => ({
  getArtistById: vi.fn(async () => null),
  parseSpotifyArtistId: vi.fn(() => null),
}));
vi.mock('./steel', () => ({
  scrapeWebsite: vi.fn(async () => null),
  normalizeUrl: vi.fn((u: string) => u),
}));
vi.mock('@/lib/gemfinder/scout-blocklist', () => ({
  isBlocked: vi.fn(async () => ({ blocked: false })),
}));
...
describe('runPipeline', () => {
  it('runs end-to-end with mocked dependencies', async () => {
    const result = await runPipeline({
      runId: 'test-run',
      workspaceId: 'test-ws',
      criteria: { genres: ['indie pop'], regions: ['US'], roleTarget: 'both', targetCount: 5 },
      weights: { /* minimal weights — fill from DEFAULT_HUNTER_WEIGHTS */ } as never,
 ...
 *   C. Gate each (gates failures recorded in summary.gatedReasons)
...
export async function runPipeline(input: RunPipelineInput): Promise<HunterRunSummary> {
  ...
  try {
    ...
    // Simple concurrency-bounded loop
    ...
    for (const sc of topN) {
      try {
        const candidate: ScoutCandidate = {
          ...
          score: sc.finalScore,
          weightSnapshot: weights as unknown as Record<string, unknown>,
          // hunter_run_id is set via a custom field in the insert query
        };
        // Note: candidate-store's createCandidate must be extended to accept
        // hunter_run_id. If not yet wired, use a raw INSERT here referencing
        // the column. See Chunk 3 store extensions.
        await createCandidate({ ...candidate, hunterRunId: runId } as never);
        summary.added++;
      } catch (err) {
        summary.errors.push({
...
```bash
npm test -- lib/gemfinder/hunter/orchestrator.test.ts
```

Expected: passes (with all mocks). Real-DB integration tests will be in Chunk 7.
...
Expected: clean. If `createCandidate` signature doesn't accept `hunter_run_id`, extend it in scout-candidate-store.ts to pass through that field.

- [ ] **Step 5: Commit**

```bash
git add lib/gemfinder/hunter/orchestrator.ts lib/gemfinder/hunter/orchestrator.test.ts
git commit -m "Add Hunter orchestrator pipeline (Chunk 4.4)

Full pipeline runner: MusicBrainz fetch → per-candidate enrichment with
concurrency=8 → gates → score → top-N → insert into scout_candidates.

Per-candidate try/catch isolation: one bad candidate doesn't fail the run.
Catastrophic failures (MB unreachable) mark run as failed.

Run summary updates are interleaved with phase completions for progress
visibility via the Runs tab UI."
```
...
**Important async pattern**: `setImmediate` returns an `Immediate` handle, NOT a Promise. The fire-and-forget pattern must wrap the promise rejection inside:

```typescript
setImmediate(() => {
  runPipeline({ runId, workspaceId, criteria, weights, actorEmail }).catch((err) => {
    console.error('[HUNTER_RUN] pipeline rejected', { runId, error: err });
    // Pipeline already handles setRunStatus('failed') internally; this catch
    // exists only to prevent unhandled promise rejection from crashing Node.
  });
});
```
...
// Inline requireEditorActor + requireScoutV3Flag per existing per-route convention
...
export async function POST(req: NextRequest) {
  ...
  // Fire-and-forget the pipeline. Pipeline handles its own status transitions.
  setImmediate(() => {
    runPipeline({ runId: run.id, workspaceId, criteria, weights, actorEmail: actor.email }).catch((err) => {
...
- [ ] **Step 2: Implement code above**
- [ ] **Step 3: `npx tsc --noEmit`** then `npm test -- app/api/ar/scout/hunter/run/route.test.ts`
- [ ] **Step 4: `npm run build`** to verify route registers
- [ ] **Step 5: Commit**

```bash
git add app/api/ar/scout/hunter/run/route.ts app/api/ar/scout/hunter/run/route.test.ts
git commit -m "Add POST + GET /api/ar/scout/hunter/run (Chunk 5.1)

POST: zod-validated criteria → createRun → fire-and-forget setImmediate
wrapping runPipeline with .catch for unhandled rejection. Returns 202
with runId.

GET: lists workspace runs, defensive stale-sweeper on each call."
```
