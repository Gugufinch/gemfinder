...
import { loadProjectsList } from './project-store';
    ...
    for (const project of projects) {
      ...
      for (const artist of artists) {
        ...
        if (identity.canonicalName && artistName) {
          const candidateModule = await import('./scout/identity');
          const artistCanonical = candidateModule.canonicalizeName(artistName);
...
async function handleApprove(workspaceId: string, candidateId: string, body: unknown, actor: { email: string }) {
  ...
  try {
    ...
    // Write workspaceScoutState[talentId] = qualified
    const projects = await loadProjectsList();
    const project = (projects as Array<Record<string, unknown>>).find((p) => p.id === projectId);
    if (project) {
      const settings = (project.settings as Record<string, unknown>) || {};
      const scoutState = (settings.scoutState as Record<string, unknown>) || {};
      scoutState[talentId] = {
        decision: 'qualified',
      ...
      settings.scoutState = scoutState;
...
- [ ] **Step 2: Add the Scout V3 card**

Insert before the existing Scout card (so it's first in lifecycle order). Use a feature-flag check:

```jsx
{selectedWorkspace?.settings?.featureFlags?.scoutV3 && (
  <div onClick={() => setScreen('scoutV3')}
       style={{ ...cS, padding: '20px 22px', cursor: 'pointer', background: C.bb }}>
    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      <span style={{ fontSize: 16, fontWeight: 800, color: C.bu }}>Open Scout</span>
      <span style={{ ...mkP(true, C.bu, C.bb), cursor: 'default' }}>
        {scoutV3Stats?.pendingCount ?? '—'}
      </span>
    </div>
    <p style={{ fontSize: 12, color: C.ts, lineHeight: 1.5, marginTop: 8 }}>
      Hunt new artists. Approve to move into Kickoff, reject to keep out forever.
    </p>
  </div>
)}
```

Also: rename the existing Scout card to "Scout V2" or repurpose to "Kickoff Triage" — this happens in Chunk 5.
...
### Task 3-6: Render functions for Queue, Rejections, Add modal, Approve modal, Reject modal, Collision modal

For each, follow the same pattern:

- Define `renderScoutV3Queue()` returning JSX that maps `scoutV3Candidates` to cards/rows with `[Approve]`, `[Reject]`, `[Edit]` buttons
- Define `renderScoutV3Rejections()` returning a read-only list
- Define `renderScoutV3AddModal()` with the manual-entry form
- Define `renderScoutV3ApproveModal()` with project dropdown defaulting to `scoutV3LastProjectId`
- Define `renderScoutV3RejectModal()` with reason dropdown
- Define `renderScoutV3CollisionModal()` to display blocklist match info

Each render function gets a commit. Use existing style helpers (`cS`, `actionBtn`, `mkP`).

**Reference the spec for exact field layouts**: `docs/superpowers/specs/2026-04-22-scout-v3-s0-s1-design.md` section "UI" lines 660-720.

- [ ] **Step 1-6: Implement each render function**

(Code shown abbreviated — implementer fills in following the spec layout exactly. Each modal is roughly 60-100 lines.)

- [ ] **Step 7: Wire approve/reject/add submit handlers**

Each handler posts to the corresponding `/api/ar/scout/*` endpoint, then updates state and closes the modal. On 409, opens the collision modal instead.
...
Expected: `/ar` route compiles. Manually verify: open workspace, click "Open Scout" (V3 card), see empty queue.

- [ ] **Step 9: Commit each piece**

Commit after each render function lands, so the diff stays reviewable.
...
In `loadProjectsList` or wherever workspace state is loaded:
...
- [ ] **Step 1: Update workspace nav**

Replace the "Open Scout" card's `onClick` to route to `screen === 'scoutV3'` when the feature flag is on. When flag is off, leave it pointing at the old Scout V2 surface.
...
import { loadProjectsList, saveProjectsList } from '@/lib/gemfinder/project-store';
