...
import { NextRequest } from 'next/server';
import { ensureSchema, createCandidate, getScoutPool } from '@/lib/gemfinder/scout-candidate-store';
import { buildCandidate } from '@/test/fixtures/scout-v3/candidates';
import { editorActor } from '@/test/fixtures/scout-v3/actors';

// Mock auth-store to return a fixture actor instead of hitting the auth DB.
vi.mock('@/lib/gemfinder/auth-store', () => ({
  getAuthUserById: vi.fn(async (id: string) => {
    if (id === 'test-editor-001') return editorActor();
    return null;
  }),
}));

function mockReq(method: string, body: unknown, opts: { id?: string; workspaceId?: string; actorCookie?: string } = {}): NextRequest {
  const url = new URL(`http://localhost/api/ar/scout/candidates/${opts.id || ''}`);
...
describe('PATCH /api/ar/scout/candidates/[id]', () => {
  it('returns 401 without auth cookie', async () => {
    const req = mockReq('PATCH', { action: 'approve', projectId: 'p1' }, { id: 'x', workspaceId: 'w1' });
    const res = await PATCH(req, { params: Promise.resolve({ id: 'x' }) });
    expect(res.status).toBe(401);
  });

  it('reject moves candidate to rejection log', async () => {
    const c = await createCandidate(buildCandidate({ workspaceId: 'w1' }));
    const req = mockReq('PATCH', { action: 'reject', reasonCode: 'wrong_genre' }, {
      id: c.id, workspaceId: 'w1', actorCookie: 'test-editor-001',
    });
    const res = await PATCH(req, { params: Promise.resolve({ id: c.id }) });
    expect(res.status).toBe(200);
    const data = await res.json();
    expect(data.rejectionId).toBeDefined();
  });

  it('approve fails with 404 if candidate already deleted (concurrent race)', async () => {
    const c = await createCandidate(buildCandidate({ workspaceId: 'w1' }));
    // Simulate concurrent deletion before our PATCH runs:
    await getScoutPool().query('delete from scout_candidates where id = $1', [c.id]);

    const req = mockReq('PATCH', { action: 'approve', projectId: 'p1' }, {
      id: c.id, workspaceId: 'w1', actorCookie: 'test-editor-001',
    });
    const res = await PATCH(req, { params: Promise.resolve({ id: c.id }) });
  ...
  // - approve happy path with project setup + verify Kickoff record created
  // - approve with re-check collision (state changed mid-flight)
  // - reject with reasonCode 'other' but no reasonNote → 400
  // - edit with blocklist collision → 409
  // - PATCH unknown action → 400
});
```
