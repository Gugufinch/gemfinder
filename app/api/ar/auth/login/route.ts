...
import { z } from 'zod';
import { loginAuthUser } from '@/lib/gemfinder/auth-store';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await loginAuthUser(parsed.data.email, parsed.data.password);
    if (!result.ok) {
    ...
    console.error('[gemfinder] auth login failed', error);
    return NextResponse.json({ error: 'Login is temporarily unavailable. Please try again shortly.' }, { status: 503 });
  }
}
