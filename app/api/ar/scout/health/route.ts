...
export async function GET(req: NextRequest) {
  ...
  // Scout feedback — training data for the adaptive-weight-learning system.
  try {
    const pool = getScoutPool();
    await pool.query('select 1 from scout_feedback limit 1');
    pass('schema.scout_feedback', 'ready');
  } catch (err) {
    fail('schema.scout_feedback', err, 'table missing; run ensureSchema()');
  }
